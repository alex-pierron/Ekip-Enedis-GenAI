...
if "geographic-distribution" in grid_items:
    with open(
        os.path.join(PROJECT_PATH, "assets/geojson/france-departements.geojson"), "r"
    ) as geojson_file:
...
dash_app.layout = dbc.Container(
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    html.Img(src="assets/img/enedis-logo.svg", height="60px"),
                    width="auto",
                    className="d-flex align-items-center justify-content-start",
                ),
                dbc.Col(
                    html.H1(
                        "Analyse des reportings Enedis",
                ...
                dbc.Col(
                    html.Img(src="assets/img/enedis-eolienne.svg", height="180px"),
                    width="auto",
                    className="d-flex align-items-center justify-content-start",
                ),
                dbc.Col(
                    html.Img(src="assets/img/hackathon-GenAI.png", height="60px"),
                    width="auto",
                    className="d-flex align-items-center justify-content-start",
                ),
            ],
        ...
        dbc.Accordion(
            [
                ...
                create_accordion_item(
                    ...
                    title="Sentiment",
                    id="tonalite",
                    ordered_values=myCSS.pie_bar_chart_colors_keys,
                ),
...
def update_visualizations(
    ...
    date_summary = (
        f"📅 Filtrer par Date: {start_date_str} - {end_date_str}"
    ...
    if "combined-pie-chart" in grid_items:
        combined_pie_chart = create_combined_pie_bar_chart(
            filtered_df, myCSS.pie_bar_chart
        )
    ...
    if "geographic-distribution" in [item for sublist in DATA_GRID for item in sublist]:
        geographic_distribution = create_geographic_distribution_map(
            filtered_df, style=myCSS.geographic_distribution, geojson=FRANCE_GEOJSON
        )
        figures["geographic-distribution"] = geographic_distribution

    if "sentiment-trend-area" in grid_items:
        sentiment_trend_area = create_sentiment_trend_area(
            filtered_df, style=myCSS.sentiment_trend
        )
        figures["sentiment-trend-area"] = sentiment_trend_area

    if "word-cloud" in grid_items:
        text = " ".join(
            filtered_df["Sujet"].dropna().tolist()
            + filtered_df["Article"].dropna().tolist()
        )
        wordcloud = create_wordcloud(
            text, style=myCSS.wordcloud, stopwords=FRENCH_STOP_WORDS
        )
        figures["word-cloud"] = wordcloud

    # Prepare return values in the correct order
    values_to_return = [
        figures.get(graph_id, {})
        for graph_id in [item for sublist in DATA_GRID for item in sublist]
    ]
    values_to_return.extend(
        [
            table_data,
            theme_summary,
            tonalite_summary,
...
@dash_app.callback(
    [Output("upload-button", "className"), Output("upload-button", "children")],
