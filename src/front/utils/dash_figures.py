...
def create_combined_pie_bar_chart(filtered_df: pd.DataFrame, style: dict) -> go.Figure:
    ...
    fig.update_traces(
        ...
        hovertemplate=(
            "<b>%{label}</b><br>" "Percentage: %{percent:.1%}<br>" "Count: %{value}"
        ),
        hole=0.4,
    ...
    for category in categories:
        ...
        for i, label in enumerate(labels):
            color = style["bar_colors"][category].get(
                label, "#000000"
            )  # Default to black
            ...
            bars.append(
                go.Bar(
                    ...
                    textposition="inside",
                    hovertemplate=(
                        f"<b>{category}: {label}</b><br>" "Ratio: %{y:.1%}<br>"
                    ),
...
def create_geographic_distribution_map(
    filtered_df: pd.DataFrame, style: dict, geojson: dict
) -> go.Figure:
    """
    ...
    aggregated_df = (
        filtered_df.groupby("Territoire")["Sentiment"]
        .agg(lambda x: x.mode()[0] if not x.mode().empty else "Inconnu")
        .reset_index()
    )
    ...
    all_territoires = pd.DataFrame(
        {
            "Territoire": [
                feature["properties"]["nom"] for feature in geojson["features"]
            ]
        }
    )
...
def create_sentiment_trend_area(filtered_df: pd.DataFrame, style: dict) -> go.Figure:
    ...
    sentiment_trend = (
        filtered_df.groupby(["Date", "Sentiment"]).size().reset_index(name="Count")
    )
